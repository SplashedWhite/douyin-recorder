...
- **每日定时开启** - 可为每个房间设置每天固定时间开启一个自动监控窗口
...
也可以在房间卡片中开启“自动录制”，或设置“每天 HH:mm”定时开启。自动录制关闭时，程序不会在后台轮询该房间；开启后会在设定的监控窗口内检查开播状态，录制期间停止轮询，关播由 FFmpeg 断流自动收尾。
...
| **单次监控窗口** | 到期仍未开播时自动停止请求（1–24 小时） | 6 小时 |
| **自动录完一场后** | 关闭自动录制或继续下一个监控窗口 | 关闭自动录制 |
